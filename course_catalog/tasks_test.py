...
TEST_JSON_PATH = './test_json/PROD/9/9.15/Fall_2007/' \
                 '9-15-biochemistry-and-pharmacology-of-synaptic-transmission-fall-2007/0'
TEST_JSON_FILES = [f for f in listdir(TEST_JSON_PATH) if isfile(join(TEST_JSON_PATH, f))]
...
def test_get_mitx_data_valid(settings):
    ...
    """
    with patch('requests.post', return_value=Mock(json=Mock(return_value={"access_token": "fake_access_token"}))):
        with patch('requests.get', return_value=Mock(status_code=200, json=Mock(return_value=TEST_EDX_JSON))):
...
def test_get_mitx_data_status_error(settings):
    ...
    """
    with patch('requests.post', return_value=Mock(json=Mock(return_value={"access_token": "fake_access_token"}))):
        with patch('requests.get', return_value=Mock(status_code=500, json=Mock(return_value=TEST_EDX_JSON))):
            settings.EDX_API_URL = 'fake_url'
            get_edx_data()
...
def test_get_mitx_data_unexpected_error(settings):
    ...
    with patch('requests.post', return_value=Mock(json=Mock(return_value={"access_token": "fake_access_token"}))):
        with patch('requests.get', return_value=Mock(status_code=200, json=Mock(return_value=TEST_EDX_JSON))):
            with patch('course_catalog.tasks_helpers.get_year_and_semester', side_effect=cause_error):
                settings.EDX_API_URL = 'fake_url'
                get_edx_data()
                assert Course.objects.count() == 0
...
def setup_s3(settings):
    ...
    for file in TEST_JSON_FILES:
        ...
        with open(TEST_JSON_PATH + "/" + file, 'r') as f:
            test_bucket.put_object(Key=file_key,
                                   Body=f.read())
