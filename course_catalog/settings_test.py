...
class TestSettings(TestCase):
    ...
    def reload_settings(self):
        ...
        """
        importlib.reload(sys.modules['course_catalog.settings'])
        # Restore settings to original settings after test
        self.addCleanup(importlib.reload, sys.modules['course_catalog.settings'])
        return vars(sys.modules['course_catalog.settings'])
    ...
    def test_s3_settings(self):
        ...
        with mock.patch.dict('os.environ', {
            **REQUIRED_SETTINGS,
            'COURSE_CATALOG_USE_S3': 'True',
        }, clear=True), self.assertRaises(ImproperlyConfigured):
            self.reload_settings()
            apps.get_app_config('course_catalog').ready()
    ...
    def test_admin_settings(self):
        ...
        with mock.patch.dict('os.environ', {
            **REQUIRED_SETTINGS,
            'COURSE_CATALOG_ADMIN_EMAIL': test_admin_email,
        }, clear=True):
            settings_vars = self.reload_settings()
            self.assertEqual(
    ...
    def test_db_ssl_enable(self):
        ...
        with mock.patch.dict('os.environ', {
            **REQUIRED_SETTINGS,
            'COURSE_CATALOG_DB_DISABLE_SSL': 'False'
        }, clear=True):
            settings_vars = self.reload_settings()
            self.assertEqual(
                settings_vars['DATABASES']['default']['OPTIONS'],
