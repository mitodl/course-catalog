...
The following commands create a Docker machine named ``course_catalog``, start the
container, and configure environment variables to facilitate communication
with the edX instance.

    docker-machine create --driver virtualbox course_catalog
    docker-machine start course_catalog
    # 'docker-machine env (machine)' prints export commands for important environment variables
    eval "$(docker-machine env course_catalog)"
...
1. Visit course_catalog in your browser on port `8073`. _(OSX Only)_ Docker auto-assigns
 the container IP. Run ``docker-machine ip`` to see it. Your course_catalog URL will
 be something like this: ``192.168.99.100:8073``.
